# Portafolio de Cursos

---

## Programas Especializados (Premium)

| Programa | Sector | Duración | Certificadora | Diferenciador |
|----------|--------|----------|---------------|---------------|
| Diplomado Internacional en Gestión de Activos | Gestión de Activos | — | Woodhouse | Alianza exclusiva |
| Programa Avanzado IAM Diploma | Gestión de Activos | — | IAM | Certificación internacional |
| Programa Profesional en Seguridad de Procesos | Seguridad | — | — | NFPA, instructor local |
| Programa Profesional en Seguridad Eléctrica | Seguridad | — | — | NFPA 70E |

---

## Cursos Estrella

| Curso | Sector | Por qué es estrella | Punto de Equilibrio |
|-------|--------|---------------------|---------------------|
| HAZOP | Seguridad | Alta demanda, genera consultoría | — |
| Seguridad de Procesos | Seguridad | Instructor local, alta rentabilidad | — |
| Seguridad Eléctrica | Seguridad | NFPA 70E, obligatorio para muchos | — |
| Comisionamiento | Integridad | OCENSA nos conoce, ventas a Ecuador | — |

---

## Catálogo Completo

### Gestión de Activos, Mantenimiento y Confiabilidad

| Curso | Estrella | Formato | Instructor | Certificación |
|-------|----------|---------|------------|---------------|
| RCA (Root Cause Analysis) | — | Ambos | — | — |
| Planificación de Mantenimiento | — | Ambos | — | — |
| Gestión de Activos | — | Ambos | — | Woodhouse |
| IAM Diploma | — | Cerrado | — | IAM |

---

### Oil and Gas

| Curso | Estrella | Formato | Instructor | Certificación |
|-------|----------|---------|------------|---------------|
| Rip Drilling | — | — | — | — |
| — | — | — | — | — |

---

### Integridad y Equipos

| Curso | Estrella | Formato | Instructor | Certificación |
|-------|----------|---------|------------|---------------|
| Comisionamiento | Sí | Ambos | — | ASME |
| — | — | — | — | — |

---

### Seguridad-Riesgos

| Curso | Estrella | Formato | Instructor | Certificación |
|-------|----------|---------|------------|---------------|
| HAZOP | Sí | Ambos | — | — |
| Seguridad de Procesos | Sí | Ambos | — | — |
| Seguridad Eléctrica (NFPA 70E) | Sí | Ambos | — | — |
| DHA (Dust Hazard Analysis) | — | — | — | NFPA 652 |

---

### Gas Natural, GLP y Energía

| Curso | Estrella | Formato | Instructor | Certificación |
|-------|----------|---------|------------|---------------|
| LNG | — | — | — | — |
| Regulación de Gas | — | — | — | — |

---

### Habilidades Directivas

| Curso | Estrella | Formato | Instructor | Certificación |
|-------|----------|---------|------------|---------------|
| Generación de Costos | — | — | — | — |

---

## Formato de Cursos

| Formato | Descripción | Cuándo usar |
|---------|-------------|-------------|
| Solo Cerrado | Solo para empresas, a medida | Cuando hay mínimo de participantes |
| Solo Abierto | Calendario público | Solo con base sólida (10+) |
| Ambos | Flexible según demanda | Mayoría de cursos |

---

## Métricas por Curso

| Métrica | Descripción |
|---------|-------------|
| Punto de equilibrio | # mínimo de participantes para rentabilidad |
| Margen objetivo | % de margen esperado |
| Tasa de repetición | % de clientes que repiten |
| NPS | Satisfacción del cliente |

---

## El Nuevo Modelo

### Antes

- Programar fechas fijas
- Promocionar a ver quién se inscribe
- Aceptar grupos pequeños (2-5)
- Descuentos sin criterio

### Ahora

- Portafolio completo sin fechas fijas
- Ir directo empresa por empresa
- Mínimos de rentabilidad definidos
- Abrir al público solo con base sólida (10+)
